class AES:
    def __init__(self):
        ...
        self.nr = 10
        self.nk = 4
        self.hex_symbols_to_int = {'a': 10, 'b': 11, 'c': 12, 'd': 13, 'e': 14, 'f': 15}
    ...
    def encrypt(self, input_bytes, cipher_key):
        state = [[] for j in range(4)]
        for r in range(4):
            for c in range(self.nb):
                state[r].append(input_bytes[r + 4 * c])

        key_schedule = self.key_expansion(cipher_key)
    ...
    def key_expansion(self, cipher_key):
        self.key_symbols = [ord(symbol) for symbol in cipher_key]
        if len(self.key_symbols) < 4 * self.nk:
            for i in range(4 * self.nk - len(self.key_symbols)):
                self.key_symbols.append(0x01)
        key_schedule = [[] for i in range(4)]
        for r in range(4):
            for c in range(self.nk):
                key_schedule[r].append(self.key_symbols[r + 4 * c])
